...
const outputPath = path.join(projectRoot, "ultimos_tests_base_datos_completos_e_incompletos.csv");
...
WITH latest_dates AS (
  SELECT DISTINCT DATE(ts."startedAt") AS session_date
...
WHERE DATE(ts."startedAt") IN (SELECT session_date FROM latest_dates)
...
async function main() {
  ...
  const wantsSsl = databaseUrl.includes("sslmode=require") || databaseUrl.includes("ssl=true");
  ...
  try {
    ...
    const summary = result.rows.reduce((acc, row) => {
      acc[row.session_status] = (acc[row.session_status] ?? 0) + 1;
      return acc;
    }, {});
